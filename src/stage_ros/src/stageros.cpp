...
#include <stage_ros/StageRosGetPoses.h>
#include <stage_ros/StageRosSetPoses.h>
...
private:
    ...
    ros::ServiceServer set_srv_, get_srv_;
...
public:
    ...
    // Service callback for set new positions
    bool cb_set_srv(stage_ros::StageRosSetPoses::Request& request, stage_ros::StageRosSetPoses::Response& response);

    // Service callback for set new positions
    bool cb_get_srv(stage_ros::StageRosGetPoses::Request& request, stage_ros::StageRosGetPoses::Response& response);
...
bool
StageNode::cb_set_srv(stage_ros::StageRosSetPoses::Request& request, stage_ros::StageRosSetPoses::Response& response)
{
  ROS_INFO("Setting new positions!");
  
  std::vector<geometry_msgs::Pose2D> poses(request.new_poses);

  if (this->positionmodels.size() != poses.size()) {
    ROS_WARN("Size of pose array different from quantity of models.");
    ROS_WARN_STREAM("Make a request with " << this->positionmodels.size() << " poses.");
    response.response = false;
    return false;
  }

  for (size_t r = 0; r < this->positionmodels.size(); r++) {
    ROS_INFO_STREAM("Setting robot " << r << " to pose " << poses[r]);
    Stg::Pose temp(poses[r].x, 
                    poses[r].y, 
                    0.0,
                    poses[r].theta);
...
bool
StageNode::cb_get_srv(stage_ros::StageRosGetPoses::Request& request, stage_ros::StageRosGetPoses::Response& response)
{
  ROS_INFO("Getting positions!");
  
  std::vector<geometry_msgs::Pose2D> response_poses(this->positionmodels.size());

  for (size_t r = 0; r < this->positionmodels.size(); r++) {

    Stg::Pose stg_pose = this->positionmodels[r]->GetGlobalPose();
    geometry_msgs::Pose2D pose_2d;
    pose_2d.x = stg_pose.x;
    pose_2d.y = stg_pose.y;
    pose_2d.theta = stg_pose.a;
    response_poses[r] = pose_2d;
  }

  response.poses = response_poses;
...
StageNode::StageNode(int argc, char** argv, bool gui, const char* fname, bool use_model_names)
{
    ...
    double t;
    localn_ = ros::NodeHandle("~");
    if(!localn_.getParam("base_watchdog_timeout", t))
        t = 0.2;
    this->base_watchdog_timeout.fromSec(t);

    if(!localn_.getParam("is_depth_canonical", isDepthCanonical))
...
StageNode::SubscribeModels()
{
    ...
    reset_srv_ = localn_.advertiseService("reset_positions", &StageNode::cb_reset_srv, this);

    // advertising set service
    set_srv_ = localn_.advertiseService("set_positions", &StageNode::cb_set_srv, this);

    // advertising get service
    get_srv_ = localn_.advertiseService("get_positions", &StageNode::cb_get_srv, this);
